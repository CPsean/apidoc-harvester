...
def sidecar_path(config_path):
    """config/<site>.yaml -> config/<site>.pages.yaml (written by --sync-pages)."""
    return os.path.splitext(os.path.abspath(config_path))[0] + ".pages.yaml"
...
def load_config(config_path):
    ...
    # pages sidecar generated by --sync-pages from a tree API (pages_from_tree):
    # normal runs only read the file — the tree API is never hit implicitly.
    sp = sidecar_path(config_path)
    if os.path.exists(sp):
        with open(sp, encoding="utf-8") as f:
            side = yaml.safe_load(f) or {}
        generated.extend(side.get("pages") or [])
    if generated:
